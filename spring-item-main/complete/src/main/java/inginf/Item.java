...
public class Item {
    ...
    public void setNomenclature(String nomenclature) {
        if (nomenclature == null || nomenclature.isEmpty()) {
            throw new IllegalArgumentException("Nomenclature must not be empty");
        }
    ...
    public void setWeightedWeight(int weightedWeight) {
        if (weightedWeight < 0) {
            throw new IllegalArgumentException("WeightedWeight must not be negative");
        }
    ...
    public void setCalculatedWeight(int calculatedWeight) {
        if (calculatedWeight < 0) {
            throw new IllegalArgumentException("CalculatedWeight must not be negative");
        }
    ...
    public void setEstimatedWeight(int estimatedWeight) {
        if (estimatedWeight < 0) {
            throw new IllegalArgumentException("EstimatedWeight must not be negative");
        }
    ...
    public void printConfStructure(ArrayList<Characteristic> configuration) {
    ...
    void printConfStructure(
        ...
        System
            .out
            .println(
                getNomenclature() + " " + getDescription() + " (" + getMaterial() + "/" +
                calcWeight() + " g)"
            );

        for (ItemInstance itemInstance : calcConfUses(configuration)) {
            itemInstance.printConfStructure(configuration, inlineCount + 1);
        }
    }
    private ArrayList<ItemInstance> calcConfUses(
        ArrayList<Characteristic> configuration
    ) {
